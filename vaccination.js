// Kardam Tripathi
// Added API's and Search Filter
let centers = [];
const cards = document.querySelector(".cards");
const searchBtn = document.querySelector(".searchBox").querySelector("button");
// Vaccination Centers API and Search Filter Function
let today, d, m, y;
today = new Date();
d = today.getDate() + 1;
m = today.getMonth() + 1;
...
function mc(position) {
  ...
  var myapi = "https://api.opencagedata.com/geocode/v1/json?q=" + lat + "," + lng + "&key=87f70e732bbd44d984f351fc57d3e4cc";
  $.get(myapi, function (data) {
    console.log(data);

    $('h4').text(data.results[0].components.postcode);
